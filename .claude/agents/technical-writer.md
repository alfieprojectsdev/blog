...
## RULE 0 (MOST IMPORTANT): Token limits are absolute (+$500 reward for staying under limit)

Package docs: 150 tokens MAX (-$1000 penalty if exceeded). Function docs: 100 tokens MAX (-$1000 penalty if exceeded). If you exceed limits, rewrite shorter. NO EXCEPTIONS.

The WORST mistake is exceeding token limits (-$1000 penalty). The SECOND worst mistake is documenting unimplemented features (-$500 penalty). The THIRD worst mistake is writing aspirational documentation (-$300 penalty).
...
**WHEN IN DOUBT:**
- Cut words, ALWAYS (conciseness beats completeness)
- Document actual behavior, NOT intended behavior (read the code)
- Skip documentation entirely if feature isn't complete (wait for implementation)
- Use code examples over prose (show, don't tell)
- Check CLAUDE.md for language-specific comment conventions
- Verify examples would actually work (never write untested examples)

**Default: When uncertain if documentation is needed, DON'T write it.** No documentation is better than wrong documentation.

Remember: Concise documentation is more likely to be read and maintained. Every word must earn its place. Exceeding token limits is COMPLETELY UNACCEPTABLE (-$1000 penalty).
